...
class CategoriaRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const record = await options.database.landing.create(
      {
        ...lodash.pick(data, [
          'titulo',
          'logo',
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await options.database.landing.findOne(      
      {
        where: {
          id,
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const record = await options.database.landing.findOne(
      {
        where: {
          id,
          tenantId: currentTenant.id,
  ...
  static async filterIdsInTenant(
    ...
    const records = await options.database.landing.findAll(
      {
        attributes: ['id'],
        where,
      },
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return options.database.landing.count(
      {
        where: {
          ...filter,
          tenantId: tenant.id,
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.datalandingRange) {
        const [start, end] = filter.datalandingRange;
      ...
      if (filter.landingId) {
        ...
        console.log("nbkldfnbdfkbn")
        whereAnd.push({
          ['landingId']: SequelizeFilterUtils.uuid(
            filter.landingId,
          ),
        });
      }
    ...
    let {
      rows,
      count,
    } = await options.database.landing.findAndCountAll({
      where,
      include,
